...
\notsay{The Hamiltonian has $N_{nn}$ terms, one for each possible nearest-neighbor bond.  
Raising it to the power $m$ would give us $N_{nn}^m$ terms each containing $m$ bond operators.}

\change{This can be avoided} by using a Monte Carlo sampling algorithm.  
If we sample terms in this sum we can \change{approximate} the ground state of the system.
The simulation \change{becomes more effective} by \change{favoring} terms that are more heavily
weighted in the \change{sum}.
\comment{Something about how we use the simulations to get the ground state wavefunction of the
system in the valence bond basis.}
