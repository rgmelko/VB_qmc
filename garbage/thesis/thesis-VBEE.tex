...
\centering
\caption[Visual example of the \vb measurement.]{
...
Also, as with \vN, $\VB = 0$ for systems with no entanglement between regions A and B.
For any single valence bond basis state (not a superposition of valence bond states) \vb and \vn agree exactly.
In one-dimensional Heisenberg spin-1/2 systems \vb seems to be in good agreement with
...
In this chapter we begin by looking at the scaling of \vb and \vn for 1D systems with open (OBC) and with periodic boundary conditions (PBC), and we compare the value of the conformal charge $c$ to the expected value for this system.
We then approach 2D, looking at $M$-leg ladder systems to determine the scaling of these entanglement entropies.  

My contribution to this project was the measurement of all of the \vb data using a single projector quantum Monte Carlo program, and the linear regression  for the \vb data.
...
(i.e. for a 1D system of length $L$, $S_{\text{A}} = S(x)$ is the entanglement entropy for a system where region A contains sites $\{1,2,\dots,x\}$ and the remainder of the sites $\{x+1,x+2,\dots,L\}$
belong to region B.
...
We use linear regression to fit the \vb and \vn data for several system sizes to Equation \eqref{cftPBC}, and Figure~\ref{c1} shows the results for the central charge $c^{\rm vN}$ approaching 1 (the expected value for this system \cite{c_is_1}) as the system size increases, however $c^{\rm VB}$ approaches a lower value. 
...
thus for even-leg ladders $S(x\gtrsim\xi) = \text{const}$. 
...
This ensures that the boundary between regions A and B cuts cleanly across all the legs of the system so that the length of the boundary is proportional to $M$, while also keeping the dividing line between the two regions far enough from the open boundaries of the system to avoid boundary effects.
...
These results also imply that the multiplicative logarithmic correction to the area law is present only in \vB, and not necessarily in all measures of entanglement entropy.
...
From the 1D results (Figs.~\ref{1dPBC} and \ref{1dOBC}) we saw that \vb can be either greater or less than \vN.
...
\rho_{\rm A} &=  {\rm Tr_B} \ket{\psi}\bra{\psi} \\ 
		...
		\end{array} \right]
 \end{align}
Diagonalizing $\rho_{\rm A}$ gives three non-zero eigenvalues all equal to $\tfrac{1}{3}$, which we use to calculate \vN,
\begin{align}
...
We also fit both entanglement entropies to the 1D conformal field theory results (Eqs.\eqref{cftPBC} and \eqref{cftOBC}), which show excellent agreement for \vN, but show significant deviation from the expected value of $c$ for \vb in the large chain length limit, approaching $c<1$ for both open and periodic boundary conditions \cite{XXX}.
...
This correction may be due to the fact that the valence bond basis is massively overcomplete.  In general, for some valence bond basis states $\psi_1$ and $\psi_2$, $\VN(\psi_1 + \psi_2) \ne \VN(\psi_1) + \VN(\psi_2)$, however we use  $\VB(\psi_1 + \psi_2) = \VB(\psi_1) + \VB(\psi_2)$ to get the valence bond entanglement entropy
...
The \vb is a reasonable measure of entanglement, that is easily accessible to scalable numerical simulations.
Our demonstration that \vb is unable to give a bound on \vN, an in particular its multiplicative log correction to the area law in 2D, make it unsuitable to characterize e.g. topologcal phases using measurements based on additive subleading corrections to area law scaling in vN, such as the topological entanglement entropy.
