...
If we define $\{\ket{\alpha^i}\}$ as a complete basis of states spanning region A, and $\{\ket{\beta^j}\}$ as a complete basis of states spanning region B, then we can represent an arbitrary state as
\begin{equation}
	\ket{\psi} = \sum_i \sum_j C^{i,j}\ket{\alpha^i}\ket{\beta^j}
\end{equation}
for some set of coefficients $C^{i,j}$.  
Then the effect of \swa on a two general states, $\ket{\psi_1}$ and $\ket{\psi_1}$ representing the two copies of the system, is
\begin{align}
	\SWA \ket{\psi_1} \otimes \ket{\psi_2}  &= 
		\SWA \left(\sum_{i,j} C_1^{i,j}\ket{\alpha^{i}_1}\ket{\beta^{j}_1}\right) \otimes
			\left(\sum_{k,l} C_2^{k,l}\ket{\alpha^{k}_2}\ket{\beta^{l}_2}\right) \\
			&=\sum_{i,j}\sum_{k,l} C_1^{i,j}C_2^{k,l} 
				\big(\ket{\alpha^{k}_2}\ket{\beta^{j}_1}\big) \otimes
				\big(\ket{\alpha^{i}_1}\ket{\beta^{l}_2}\big)
\end{align}
