...
\begin{abstract} 
We develop a procedure for measuring the Renyi entanglement entropy of a subregion $A$ in a quantum
many-body system using valence-bond basis quantum Monte Carlo simulations as the
to the expectation of a unitary operator which swaps the 
basis states of a region $A$ with an ancillary copy of that region.  We present an algorithm for
calculating the Renyi entropy directly from this {\it Swap} operator, using an improved estimator
involving the ratio of {\it Swap} operators for different subregions which enables us to converge
the estimate with a number of samples that appears to scale only polynomially in the region size.
...
new tools and insights to the field of condensed-matter physics.  Conformally invariant
systems in one dimension display universal logarithmic
scaling of entanglement\cite{Cardy}.  Many
quantum systems in two dimensions (2D) and higher are predicted to have an area law with
...
Thus, the study of entanglement entropy promises to provide an important new 
...
is ony accessible numerically when the reduced density matrix can be calculated directly, such as Lanczos 
...
We resolve this problem and show that a generalized Renyi entropy, $S_2$ below, can be represented in the valence-bond basis as the expectation
...
{\it Discussion.}-- 
We have presented the first measurement of Renyi entanglement entropy in two dimensional systems, confirming the area law.
Using the improved ratio estimator, we were able to converge the expectation value of the swap operator.

It is known that, by using loop algorithms, it is possible to compute observables such as energy and spin correlation functins
for systems much larger than $L=28$.  The loop algorithms significantly speed up the ROGER, CAN YOU WRITE SOMETHING HERE?
In future work, we will implement a loop algorithm using the improved ratio estimator.
The question naturally arises: will it still be possible to converge the expectation value of the swap operator using
only polynomially many sample?  After all, at the largest system sizes we considered, $r=2$ was insufficient for convegence.
However, it is possible to change the sequence of regions $A^i$ that we use.  Rather, than considering a sequence of regions $A^{\ell}$ consisting
of $\ell$-by-$\ell$ squares, we can use a more finely grained sequence of regions.  For example, 
we can define $A^i$ to contain
$i$ sites such that for $i=\ell^2$, $A^i$ is an $\ell$-by-$\ell$ square.  Since at each stage we only change the region locally, building up  a square of size $\ell+1$ by adding sites one at a time around the edge, we expect that
this will require only a polynomial number of samples for convergence for arbitrary system sizes.
