...
(a) and after (b) $Swap_A$ operation. % in the valence bond basis.  
The
region $A$ consists of three light-colored sites on the left; the
complement region $B$ of three dark sites on the right. Curvy lines denote
singlets. $|V_{r}\rangle$ is a product of two (one \textit{per} copy
and in general different) valence bond states. The ground state of the
two-copy system is a linear combination of such $|V_{r}\rangle$.
}
} \end{figure}
...
This is an indication that simulation ergodicity may be jeopardized by low sampling of the $Swap_A$ operator.%, similar to that observed in 1D.
