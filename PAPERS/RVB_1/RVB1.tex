...
%CFTs. 
%Although traditional methods of finding the 
%The central charge can be numerically determined in 1D quantum systems by finding the finite-size
% spectrum \cite{BCN,Affleck}, but 
%this is a formidable task: not only is $c$ a subleading contribution to the ground state energy, but %extracting it also requires the knowledge of a velocity.
%
...
%Due to this result, numerical measurements of the central charge have become commonplace
% and very accurate.
%, e.g.\ in simulations using the density matrix renormalization group \cite{White92,Scholl05}. 
%This has made Eq.~(\ref{1Dcft}) the gold standard for identifying $c$.
...
CFT \cite{Moore06,Hsu08,Misguich}.  In the presence of a spontaneously broken
...
The purpose of this
paper is to analyze one type of universal term occurring in two-dimensional (2D) gapless systems. Gapless modes typically have long-range correlations, so it is possible for
...
In both cases, we find a shape-dependent scaling function that closely mimics the chord-length contribution in 1D in Eq.~(\ref{1Dcft}).

To probe this behavior in a simpler system, we also study
 free spinless fermions in the $\pi$-flux phase and find that the entanglement scaling also has a universal shape-dependent piece.
...
Among other consequences, this shape-dependent term will give a non-zero signature in the entanglement quantities \cite{KP,LW} designed to look for topological order, which complicates any possible generalization of the topological entanglement entropy to gapless spin-liquid states. 
...
\smallskip
\paragraph{ Discussion---} We have studied the Renyi entanglement entropy in the ground state of three gapless systems on $L_x\times L_y$ toroidal lattices, where the subregion $A$ is a cylinder of length $x$.  We have  
...
Interestingly, just as strong sub-additivity constrains the sign of the Levin-Wen entropy \cite{LW}, it also implies that for fixed $L_x,L_y$ that $\gamma$ for the von Neumann entropy is a concave-down function of $x$.
