...
\begin{abstract} 
We numerically determine \textcolor{red}{\sout{universal}} \textcolor{blue}{subleading scaling} terms in the ground-state entanglement entropy of several two dimensional (2D) gapless systems, including a Heisenberg model with N\'eel order, a free Dirac fermion in the $\pi$-flux phase, and the nearest-neighbor resonating-valence bond wave function.
...
is a more general \textcolor{red}{\sout{universal}} function in the 2D thermodynamic limit.
...
The purpose of this
paper is to analyze one type of \textcolor{red}{universal} term occurring in 2D gapless systems. Gapless modes typically have long-range correlations, so it is possible for
...
\paragraph{Fermions with $\pi$-flux---}
...
We can however further examine the deviation from conformal-style scaling by examining the \textcolor{red}{\sout{universality of the} }
...
This functional dependence is apparently sub-linear \textcolor{blue}{ -- possibly behaving like $c(L) \sim L^p$ with $p\leq1$.  However, in order for this shape-dependent term to be universal in 2D, one expects $c$ to approach a constant in the limit $L \rightarrow \infty$.
That scenario could be supported by the QMC data if convergence were assumed to be very slow.  Distinguishing between these
two scenarios (and therefore the strict adherence of $\gamma$ to universality in this system) is impossible on the system sizes studied.}
%A rigorous analysis of the data along these lines is impossible however within the current
...
$x$ are excluded. It is worth noting that in the closely related quantum dimer
model on the square lattice, no such term appears: in this geometry,
the only subleading term in the entanglement entropy is constant 
\cite{Hsu08,Misguich,Oshikawa,Hsu10,Zaletel,Stephan11}.
...
limit. 
\smallskip
\paragraph{ Discussion---} We have studied the Renyi entanglement entropy in the ground state of three gapless systems on $L_x\times L_y$ toroidal lattices, where the subregion $A$ is a cylinder of length $x$.  
%We have  
...
{\it Acknowledgments---} 
The authors thank J.-M. Stephan, A.~Del Maestro, E.~Fradkin, I.~Klich, K.~Intriligator,  M.~Metlitski, E.G.~Moon, R.~Myers and A.~Sandvik for enlightening discussions. 
