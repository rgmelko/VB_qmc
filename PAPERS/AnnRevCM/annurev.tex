...
% there is no sign problem, or it is known how to circumvent it, 
...
any particular real material (although some times that is also possible). The idea is to design a sign-problem free Hamiltonian in such a way that it contains 
...
very narrow regions of critical scaling. This aspect of criticality in dimerized models have also been investigated in detail in QMC 
...
absence of four-fold symmetry implies that here the system size $L=32 < \Lambda$. By analyzing the cross-over from Z$_4$ to U($1$) symmetry using 
finite-size scaling techniques, one can extract the exponent governing the length-scale $\Lambda$, with the result that $\Lambda \sim \xi^{1+a}$, with $a 
\approx 0.2$ \cite{lou2009:sun}.
...
that the exponent $\eta_N$ for the SU($2$) case has been estimated based on classical Monte Carlo simulations of the non-compact CP$^1$ model \cite{Motrunich04}. 
...
It is well known that the Berry phases cancel in the long-wavelength limit between the layers. Nevertheless,  both N\'eel and VBS phases are clearly stable to small finite interlayer coupling.
...
Landau paradigm, due to the cancellation of Berry phases which is relevant perturbation at the deconfined critical point \cite{kaul2012:sun_bil}.
 This is another piece of strong evidence 
for the correctness of the DQC theory, as well as the more general field-theoretical understanding of the role of Berry phases.
...
such sign-problem free U($1$) models. In this section, we will explore one class of spin Hamiltonian that contains the simplest type of QSL phase, which can be related to a 
...
%These examples are part of a broader class, originally identified to harbor the simplest Z(2) spin liquid phase at an exactly soluble RK-point, was introduced by Balents, Fisher and Girvin.\cite{BFG}  
...
and exploring various quantum many-body phenomena. Here we discuss some further generic aspects of this approach, followed by an 
...
can apply more sophisticated ways to circumvent the sign problem, e.g., the Meron algorithm \cite{Chandrasekharan99} or the newly discovered fermion bag approach~\cite{chandrasekharan2010:fbag}, for a wider range 
of fermionic and frustrated  models.

\subsection{Future prospects}

Within the classes of models we have discussed here, although the essential physics is now has been ascertained, there still remains
...
effects of dilution on quantum-critical scaling have also been observed that still lack explanation and deserve further study \cite{Sandvik06,Yao10}. 
