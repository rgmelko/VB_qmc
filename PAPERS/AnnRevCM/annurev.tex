...
values $n\pi/2$ when the order parameters $D_x$ and $D_y$ are averaged over regions of size $\gg \Lambda$. There are many interesting consequences of the
approximate U($1$) nature of the VBS order parameter, as further discussed in Ref.~\cite{Sandvik12}.
...
spinons using a designer hamiltonian with a VBS state and a N\'eel--VBS transition in one dimension \cite{Tang11a}. Similar calculations for 2D J-Q models 
should shed further light on the nature of the spinons at the DQC point.
