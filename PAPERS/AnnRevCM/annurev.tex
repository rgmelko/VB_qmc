...
In Fig.~\ref{synergy} we illustrate the synergistic role of such studies in combination with experiments and field-theory
approaches to uncover universal physics in condensed-matter systems.

\begin{figure*}
\centerline{\psfig{figure=fig01.eps,angle=-90,height=11cm}} 
\caption{Illustration of synergies between unbiased numerical (here QMC) studies of lattice Hamiltonians, field-theories for
describing asymptotic low-energy physics, and experiments on real materials. By combining all three approaches a complete picture of universality
in strongly-correlated and entangled quantum systems can emerge.}
\label{synergy}
\end{figure*}
...
from the area of quantum magnetism. In Fig.~\ref{rfig} we illustrate some of the quantum states that can be accessed with designer quantum spin 
Hamiltonians. Of particular interest here are states that have no natural classical analouges, e.g., spin liquids, which can host topological 
order \cite{Wen03}, and valence-bond-solids (VBSs), where crystalline order is formed not among the original degrees of freedom appearing in the 
Hamiltonian, but among emergent objects---localized singlets of two (or more) spins.

An important aspect of research on designer hamiltonians is significant advances in QMC methods \cite{Sandvik91,Evertz93,Beard96,WormA,Sandvik99,Sandvik10a},
which have made it possible to truly approach the  low-energy limit of many important models without any approximations (beyond controllable statistical errors), 
and to compute quantities of current interest in condensed matter physics and quantum information theory, e.g., the fidelity 
...
\begin{figure}
\centerline{\psfig{figure=fig02.eps,height=3.5cm}}
\caption{(Color online) Ground states of quantum spins on the square lattice. The left panel shows an antiferromagnetically ordered (N\'eel) state, which is
a direct generalization of a classical state (with quantum fluctuations added to a fully ordered state). The middle and right panels illustrate manifestly
non-classical states with no magnetic order. These states have a natural description in terms of singlets (the ovals). In a spin liquid (middle), the singlets
fluctuate and there are no broken symmetries, while in the VBS there are residual interactions between the singlets, leading to crystalline
order.}
\label{rfig}
\end{figure}
...
non-compact CP$^{N-1}$ description of the intriguing  N\'eel to VBS ``deconfined'' quantum-critical point in 2D 
...
$\beta$ is dominated by powers of the order $n=\beta |E_0|$, where $E_0$ is the ground state energy.
...
\cite{Sandvik10a}. An example of a configuration in such a simulation is shown in Fig.~\ref{loops}. This configuration space is amenable to the very efficient 
...
by making the transformation $|\uparrow \rangle\rightarrow |\downarrow\rangle, |\downarrow \rangle\rightarrow -|\uparrow\rangle$ on one of the sublattices. 
We can use  $|s^N_{ij}\rangle$ to rewrite the manifestly SU($N$) invariant version of Eq.~(\ref{psinglet}) as an explicitly sign-problem 
...
\begin{figure}
\centerline{\psfig{figure=fig05.eps,height=4.5cm}}
...
\begin{figure}
\centerline{\psfig{figure=fig06.eps,height=9cm}}
...
(with $L_0=0.9$) in order to compensate for a weak drift of the crossing points. The red curves, which cross at a single point, the critical point $q_c=(J/Q)_c = 0.0447(2)$, represent a common scaling function for $L\ge 48$ (approximated by a common polynomial fitted to all the data points), $f[(q-q_c)L^{1/\nu}]$, 
with $\nu=0.59$.}
\label{jqrhos}
\end{figure}
...
\subsubsection{$T>0$ critical scaling} 

The quantum-critical ``fan'' in the $(J/Q,T)$ plane has also been investigated. Initial calculations confirmed that
...
\begin{figure}
\centerline{\psfig{figure=fig07.eps,height=6cm}}
...
discussed above. Secs.~\ref{ss:j1N}, \ref{ss:jqN}, \ref{ss:j1j2N}, and \ref{ss:j1j2q} describe several studies carried out on a single 
layer, and Sec.~\ref{ss:bilN} summarizes a recent study on a bilayer. 
...
\begin{figure}
\centerline{\psfig{figure=fig08.eps,height=11cm}}
...
We here discuss some aspects of the critical behavior for $N \in \{2,3,\ldots,12\}$ obtained in these calculations and relate the results to 
...
\begin{figure}
\centerline{\psfig{figure=fig09.eps,height=9cm}}
...
\begin{figure}
\centerline{\psfig{figure=fig10.eps,height=9cm}}
...
\begin{figure}
\centerline{\includegraphics[width=4.5in]{fig11.eps}}
...
\end{equation}
 Here, the subleading term to the boundary law is a universal correction called the {\it topological entanglement entropy} \cite{Alioscia1,Alioscia2,LW,KP}.  In a gapped quantum spin liquid phase, it is related to the emergent gauge symmetry, independent of the Renyi index $\alpha$ \cite{Flammia}.  In particular, in the $Z_2$ spin liquid state, $\gamma =  \ln(2)$ at $T=0$ \cite{LW}.   The addition of this universal subleading constant to the boundary law motivates the notion of LRE for a gapped quantum spin liquid phase.

\subsubsection{Calculating the topological entanglement entropy}
...
\begin{figure}
\centerline{\includegraphics[width=4.5in]{fig12.eps}}
...
model model \cite{compositefieldtheory,compositeMC,Vicari}, leading to $\eta\approx  1.472(2)$.
...
generic aspects of this approach, followed by a brief discussion of other interesting related studies of systems not covered in this 
Review, and an outlook on future directions.

\subsection{Designer Hamiltonian, stoquasticity, and the sign problem}
...
Regarding the sign problem itself more broadly, Ref.~\cite{Troyer05} presented a proof that the sign problem cannot be solved in general.
The problem here was defined in terms of constructing a QMC algorithm for which the computational effort scales as a power-law in the system
size. A particular problem is considered for which the removal of the negative signs still leaves a problem for which the Monte Carlo sampling is believed
to scale exponentially in the system size and, thus, the sign problem was not solved, only recast into a different form. However, the model system chosen in
...
Some non-trivial solutions of sign problems are worth mentioning. In some cases, local basis transformations can be used to render all off diagonal matrix 
elements non-positive \cite{Nakamura97}. One can some times apply more sophisticated ways to circumvent the problem, e.g., with the Meron algorithm 
\cite{Chandrasekharan99} or the newly discovered fermion bag approach~\cite{chandrasekharan2010:fbag}. While some interesting applications of these methods 
have appeared~\cite{chandra2012:thirring}, it remains to be seen whether these exciting developments can be applied to a wider range of fermionic and 
frustrated designer Hamiltonians. {\color{red} Fermion QMC methods have also been developed where the signs are completely removed, but after initial optimism 
about these ``Gaussian QMC'' methods \cite{Corney04}, it appears that the exponentially increasing computational effort is transferred in a non-trivial way 
to the sampling space, with broad tails appearing in the probability distribution that destroy the normal favorable scaling properties of importance 
sampling \cite{Corboz08}.}
...
SU($N$) QMC simulations with large-$N$ expansions, summarized in Sec.~\ref{ss:j1j2q} and Fig.~\ref{fig:exp}, it would be very useful to go to still larger lattices, 
...
introduces a sign problem, it should be possible to study large lattices doped with a small number of holes, since the magnitude of the 
...
{\color{red} Large-scale QMC studies of some sign-problem free fermionic models with interesting ground states phases and quantum critial points are possible.
Hubbard models have no sign problems within determinant-based methods in particle-hole symmetric cases---on bipartite lattices at 
half-filling \cite{hirsch85,Assaad07}---and also in the presence of field-terms causing a transition between a Mott insulator and a d-wave superconductor 
\cite{assaad96}. SU($N$) generalizations with even $N$ are also free from sign problems and have been studied \cite{Assaad05}. 

While the standard Hubbard 
model on the square lattice is always Mott-insulating, due to nesting \cite{hirsch85}, in the the bipartite honeycomb-lattice Hubbard model a critical value 
of the on-site interaction $U$ is required for driving a transition from a semi-metal to the antiferromagnetic Mott insulator. This quantum phase transition 
has also been studied with  QMC \cite{Sorella92,Paiva05}. Interestingly, a recent study found evidence for a gapped spin liquid between the semi-metal and the
N\'eel state  \cite{Meng10}, and this spurred intense activities to deduce the exact nature of this state \cite{clark2011:honey,lu2011:honey,wang2010:honey,Ran11}. 
However, A subsequent QMC study on larger lattices \cite{Sorella12} has cast doubt on the extrapolations on which the existence of the spin liquid had 
been claimed, arguing instead that a system satisfying Marshall's sign rule (in the spin sector) should never host such a state. 

Very recently \cite{berg2012:mqcp}, a sign-problem free quantum action was identified in which a bosonic order parmater condenses 
in the background of a Fermi surface. This opens up the exciting possibility 
of unbiased studies of magnetic critical points in a metal, a topic of long-standing interest in condensed matter physics \cite{vojta2007:rmp}.}
...
investigated with QMC in the 2D and 3D N\'eel states \cite{Hoglund04,Anfuso06,Liu09} and at conventional quantum-critical points in dimerized models \cite{Hoglund07}. 
...
A recent development on the methods front is the adaptation of QMC algorithms such as those discussed in Sec.~\ref{sec:methods} to {\it non-equilibrium} 
...
exponent. Moreover, the fidelity and full geometric tensor, which characterize the geometrical properties of the state space, can be accessed 
this way. One can anticipate many fruitful applications of this unconventional QMC approach, which gives access to dynamics beyond the commonly 
used numerical analytic continuation of QMC-computed equilibrium imaginary-time correlation functions \cite{Syljuasen08}.
...
We also gratefully acknowledge support from the NSF I2CAM International Materials Institute Award, Grant DMR-0844115, as well as the International Center for Theoretical
Physics (ICTP), Trieste, Italy, for support for the workshop {\it Synergies between Field Theory and Exact Computational Methods in Strongly Correlated Quantum Matter},
at ICTP in July 2011, where the foundations for this article were laid.
