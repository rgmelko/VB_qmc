...
In view of issues such as these, it is important to test the predictions of field theories in some unbiased way,
...
While there has been some progress for 2D systems with DMRG \cite{Stoudenmire12} and tensor-product 
...
The only numerical approach with which one can routinely reach sufficiently large 2D and 3D lattices is QMC simulations 
\cite{Evertz03,Sandvik10b}, but, as already noted, they are restricted to systems free from sign problems. The class of models 
...
scheme implemented), but often not one that would have come natural as a starting point just from the perspective of increasing the 
...
by a ``deconfined'' quantum-critical (DQC) point \cite{Senthil04a} corresponding to the {\it non-compact} CP$^1$ field throry (in contrast to the nonlinear 
$\sigma$-model, which is a description equivalent to the {\it compact} CP$^1$ action, i.e., the  Berry phases make the gauge field non-compact).
It describes spinons weakly interacting with a U($1$) gauge field  The VBS and N\'eel order parameter form due to confinement and condensation, 
...
The gauge field in the critical theory is U($1$) symmetric, but in the $J$-$Q$ model such a symmetry is not apparent in the Hamiltonian; it
...
to investigate the convergence of the computed exponents. This is important, in particular, in light of the fact there there are significant scaling 
...
Another very interesting aspect of the designer hamiltonians is that they can be doped with fermions. While this introduces a sign problem, it
should be possible to study large lattices doped with a small number of dopants, since the magnitude of the sign problem only depends on the number
of fermions. This low-density limit should already be sufficient for making connections with theorises of ``holonic'' or ``strange'' metallic states,
that have been predicted to arise out of the DQC system upon doping.

An important open question is whether one can more sophisticated ways to circumvent the sign problem, e.g., the Meron algorithm, for a wider
range of fermionic models.
An alternative conceptual approach may be suggested by the demonstrated existence of the $Z_2$ fractional phase in a purely bosonic model, which raises the possibility that, with a suitable ``binding'' potential \cite{FermionBind}, an extension of the model with an emergent fermionic excitation might be created \cite{Wenbook}.  
...
could also have wide implications for the purported application of tensor network algorithms as a ``solution'' to the sign problem, if such a state were found to violate the area law (Equation \ref{arealaw}). 
