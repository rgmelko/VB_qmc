...
the projected bra and ket states. Estimators for various correlation functions are discussed in Refs.~\cite{Beach06} and \cite{Tang11b}.
...
understand in the context of the projector algorithm discussed above in Sec.~\ref{su2method}.  Here, the second-Renyi entropy (for example) is given by the
 mid-point evaluation of a {\it Swap}$_A$ operator {(\color{red} would ${\rm SWAP}_A$ look better?)}
...
of the {\it Swap}$_A$ operator can be replaced by an evaluation of the difference in free energies between a replicated system (similar to Fig.~\ref{swap}), 
