...
Standard QMC algorithms can be classified into {\it finite-temperature} methods, where the goal is to compute a thermal
average of the form
\begin{equation}
\langle A\rangle = \frac{1}{Z}{\bf Tr}\{ {\rm e}^{-\beta H}\},~~~~
Z = {\bf Tr}\{ {\rm e}^{-\beta H}\},
\end{equation}
and {\it ground-state projector} methods, where some operator $P(\beta)$ is applied to a ``trial state'' $|\Psi_0\rangle$,
such that $|\Psi_\beta=\rangle = P(\beta)|\Psi_0\rangle$ approaches the ground state when $\beta \to \infty$ and an
expectation value 
\begin{equation}
\langle A\rangle = \frac{1}{Z}\langle \Psi_\beta|A|\Psi_\beta\rangle,~~~~ Z = \langle \Psi(\beta)|\Psi(\beta)\rangle,
\end{equation}
approaches the true ground state expectation value, $\langle A\rangle \to \langle|0| A|0\rangle$. For the projector, one can use the imaginary-ime
evolution operator (\ref{utau}), $P(\beta)=U(\beta)={\rm e}^{-\beta \mathcal{H}}$ with a fixed (time-independent) hamiltonian, or one can use a high
power of the hamiltonian, $P(\beta)=H^m$, where $\beta=m/N$ gives roughly the same rate of convergence for the two choises for a given system size 
$N$. This follows from a Taylor expansion of the time evolution operator, which for large $\beta$ is dominated by powers of the order
$n=\beta |E_0|$, where $E_0$ is the ground state energy (which is proportional to $N$) .
...
Much of our understanding of QSL phases comes from their relation to {\it quantum dimer models}.  Such models have a strong local constraint that the number of dimers emerging from a site is fixed, as well as quantum tunnelling that allows the state to fluctuate while keeping the constraint satisfied.  Under special circumstances, dimer Hamiltonians can be constructed where the fluctuating dimer configuration breaks no symmetry -- being a type of QSL (or resonating valence bond) phase.  The features that emerge in this phase can be directly related to special field theories with gauge symmetry and topological order.

Such dimer models give us a recipe to construct QSL phases in spin or boson models.  The philosophy is to map the dimer constraint to a plaquette spin configuration on the dual lattice; the dimer may represents e.g.~an $S^z$ spin or boson density $n$ configuration on a bond which is frustrated (or unsatisfied).  Then, the role of the quantum dimer move which tunnel between dimer configurations is done by quantum hopping (spin exchange) - processes which are often higher-order in $t$.  Such models are called
...
H &=& -t \sum_{( ij )} [b^{\dagger}_i b_j + b_i b^{\dagger}_j] + V \sum_{\hexagon} (n_{\hexagon})^2 
\end{eqnarray}
