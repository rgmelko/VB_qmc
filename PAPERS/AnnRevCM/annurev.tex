...
of these technical aspects as well. We here first give some further remark on field theory and the philosophy of the 
...
variables without diagonalizing $H$. In practice, implementations of $T=0$ and $T>0$ approaches for a given model are often very similar. In either case,
the exponential operator can be treated with path-integral methods (world lines in discrete or continuous imaginary time), starting from a product of
...
imaginary time. Older approaches employed the Suzuki-Trotter decomposition in (\ref{slicing}), typically leading to an error $\mathcal{O}(\Delta^2)$.

Al alternative way is to start from a series expansion,
\begin{equation}
...
in $V$. The full SSE is normally more efficient for spin systems, while the continuous-time variant should be better for certain boson systems.
