...
hand, quantum field theory is a powerful and general framework for studies of low-energy properties of many complex, strongly correlated and entangled 
...
of phase transitions {[\color{red} Refs]}. 
...
where one  can obtain unbiased results for generic nontrivial models. There are encouraging signs, however, of calculations addressing challenging systems and 
...
classical model with the same global symmetries in $d+1$ dimensions \cite{Rieger94,Sorensen92,Nahum11} (e.g., the quantum-critical point of the transverse field 
Ising chain is described by the ($1+1$)-dimensional Ising field theory \cite{Sachdev11}) {\color{red} [Is it just the critical point? Is this ft also
not describing the two phases?]}, in other cases this naive mapping can miss important and intriguing 
...
We will here only give a brief overview of the methods, with references to more detailed accounts.
...
of the $\alpha$ times replicated system), with the operator $A$ replaced by a ``swap'' \cite{Hastings10} or ``permutation'' operator \cite{Kallin11}. This is easiest to 
...
where designer Hamiltonians can give unique insights. To mention one example where the standard classical--quantum correspondence fails, in the 2D $S=1/2$ 
...
excitations not anticipated based on the $d$ $\to$ $(d+1)$-dimensional mapping. More on randomness.... cite T. Vojta. Also single-impurity physics.
