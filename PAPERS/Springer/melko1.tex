...
\end{equation}
where one aims to use some procedure find $\Psi$ as the ground-state wavefunction of a Hamiltonian.  
...
For the Heisenberg (or other SU(2) Hamiltonians) a particularly convenient class of trial states are amplitude-product states in the ``valence-bond'' (VB) basis, which is single-pair coverings of a (bipartite) lattice.  That is, 
\begin{equation}
...
With this, a projection QMC scheme satisfying Equation \ref{normZ2} can quickly be constructed by noting that the projection operator serves to re-arrange the end-points of the valence bonds.  The resulting picture becomes very much like the finite-temperature SSE, where up-spins propagating from $\langle \alpha_{\ell} |$ to $| \alpha_r \rangle$ form lines that are re-arranged by the $2m$ operators that occur in the expansion, Equation \ref{Zproj2b}.
...
\begin{enumerate}
\item If a diagonal operator $H_{1,a}$ is present on bond $a$, attempt to move it with probability ...{\bf ANN!}
\item If an off-diagonal operator $H_{2,a}$ is present on bond $a$, do not change it.
\end{enumerate}
...
\end{equation}
where ${\bf \sigma}_i$ is a Pauli spin operator, so that $\sigma^z$ can be $\pm 1$.  In this equation, the first sum is a sum over lattice {\em bonds}, while the second sum is over lattice sites.  An SSE procedure for simulating the TFIM was first developed by Sandvik for the finite-$T$ representation; this was subsequently adopted to a $T=0$ projector method, discussed in section XXX.
...
\langle \hspace{1mm} \circ \hspace{1mm}  \circ \hspace{1mm}  | H_{1,a} | \hspace{1mm} \circ \hspace{1mm} \circ \hspace{1mm} \rangle = 1 \label {Tfimdiag} 
\end{eqnarray}
...
\item The algorithm proceed to the next propagation step in $M$, and returns to step (1).
\end{enumerate}
