...
\end{enumerate}
Throughout this update, the spin state should be propagated from $\langle \alpha_{\ell} |$ to $| \alpha_r \rangle$ in a way consistent with the off-diagonal operators.  Otherwise, one would sample a normalization weight, Equation \ref{Zproj2b}, equal to zero.
...
P(h) = \frac{h N}{hN + (2J)N_b}
\end{equation}
where $N$ is the number of sites, and $N_b$ the number of bonds.  Note, $P(J) = 1- P(h)$ as before.  %Comparison to a random number between 0 and 1 decides which operator type is chosen.
\item If $H_{-1,a}$ is chosen, a site $a$ is chosen at random, and the operator is placed there.
...
%\subsection{Unification of finite-$T$ and $T=0$ methods}

%\subsection{The future of SSE}
