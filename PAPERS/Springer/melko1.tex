...
\begin{enumerate}
\item Traverse the operator list $\prod_{i=1}^{M} {H_{t_i,a_i}}$.
\item If an off-diagonal operator $H_{2,a}$ acting on a bond $a$ is encountered move on to the next operator.
\item If a diagonal operator $H_{1,b}$ is present on a bond $b$, pick a random bond $c$ to move it to.
\item If bond $c$ does not have antiparallel spins, choose another random bond.
\item Repeat this process until the diagonal operator is successfully placed.
