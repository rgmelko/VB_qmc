...
\subsection{The Coulomb-gas approach}
...
An important subtlety in this mapping is the boundary conditions. 
With periodic boundary conditions and an even number of sites around this cycle (as in the left and right sides of the figure \ref{fig:height_shift}), note that means that heights can jump by $2\pi$ times an integer on from one side to the other. %This is the definition of the compactification $h\sim h + 2\pi$ in the field theory. >>> We should include the last column of face variables in the figure??? 
With open boundary conditions, as we have at the ends of the cylinder (the top and bottom of the figure), the natural definition of the boundary height comes from averaging the heights along the boundary. For a cylinder of even $L_x$ circumference and height $L_y$, it is easy to check (see Fig.~\ref{fig:height_shift}) that the coarse-grained height difference between top and bottom satisfies \cite{Ferdinand}
\begin{equation}
 ...
 \right.
\end{equation}
This difference in behavior will result a striking even/odd effect in the two-cylinder entanglement entropy. 
 
The standard Coulomb-gas arguments \cite{Nienhuis} then imply upon coarse-graining the height variable $h(i,j)$ into a continuous field $h$, the scaling limit of the classical dimer model is a free scalar field $h(x,y)$ with euclidean action
 \begin{equation}\label{eq:free_field_bis}
  S_\kappa[h]=\frac{\kappa}{4\pi}\int \left(\nabla h\right)^2 dx dy,
 \end{equation}
The coupling $\kappa$ is often known as the ``stiffness''. Such a field theory description is also known as a ``Coulomb Gas'' \cite{Nienhuis}, and is one of the simplest examples of a two-dimensional conformal field theory \cite{Ginsparg}. It is ubiquitous in two-dimensional statistical models, as well as in condensed matter, where it describes Luttinger liquids.  In order to be consistent with periodic boundary conditions, field values shifted by $2\pi$ must be identified; in conventional language it is said that the field $h$ is compactified on a circle of radius $1$: $h\sim h+2\pi$. 
...
Once the stiffness fixed to its right value, Eq.~(\ref{eq:free_field}) describes all the universal properties of the classical dimer model. By virtue of the correspondence between correlators in the classical model and those in the dimer RVB state, the same exponents apply to the correlators in the latter as well.
...
>>> JM: A few words about the quantum 6v model, and/or interacting dimers???
...
\section{Two-cylinder Renyi entropy from CFT partition functions}
...
\begin{equation}
 ...
 \label{Spsm}
\end{equation}
Note that here $n$ need not be an integer.

This expression makes it possible to use the computer to find very accurate values of the Renyi entropy for the dimer RVB state. Using free-fermion/Pfaffian techniques described in \ref{sec:lgv}, each probability can be expressed as a product of determinants of two matrices of size $\sim L/2$, and computed numerically in a time of order $\approx L^3$. Using translational symmetry along the $x$ axis as well as the conservation of winding numbers, we are able to compute any R\'enyi entropy up to machine precision in a time of order $L^{3/2}\times 4^L$.
...
(\ref{eq:shape_general}) for $n>n_c$. This follows fairly simply from the description of the long-distance behavior of classical dimers in terms of the free boson action (\label{eq:free_field_bis}).
...
If such a term is relevant, then the action flows to a configuration where the height $h$ is locked to the values $h=h_0$ mod $2\pi$. For dimers on the square lattice, it has long been known that any such term allowed are irrelevant in the bulk; this is why the action (\label{eq:free_field_bis}) applies and the correlators are algebraically decaying. However, we showed in the preceding subsection \label{sec:eeshannon} that the probabilities used to compute the Renyi entropy depend on the boundary conditions. Thus the crucial observation of  \cite{Stephan2011,Moore} is that  one needs to include terms of the form (\ref{Vdef}) localized at the {\em boundary}.
...
\begin{equation}
 n_c=1.
\end{equation}
The two qualitatively different types of behavior seen in fig.\ \ref{fig:ee_d_torus} indeed occur on opposite sides of $n_c$. It is important to notice that even though $S_n$ should take an universal form in both phases, the value itself of the critical R\'enyi parameter depends on a degeneracy $d_{\rm min}$, which is non-universal. For example, $d_{\rm min}=2$ for the quantum six-vertex wave function, and $d_{\rm min}=3$ for quantum dimers on the hexagonal lattice. 

This observation now allows us to derive the expression for $S_n$ valid for $n>n_c$. In this phase, the boundary operator is relevant. This means that here the boundary value of the field locks on to a minimum of $V_1$. This minimum must be fixed along the entire boundary; this is commonly known as Dirichlet boundary conditions on the boson. We refer therefore to the phase with $n>n_c$ as the ``locked'' phase.
...
In the continuum, these partition functions become those of the free-boson conformal field theory with fixed Dirichlet boundary conditions at the ends of the cylinder, so we have now derived (\ref{eq:shape_general}) with boundary conditions now specified.


For the dimer RVB state, the partition functions on the cylinder can be computed exactly both on the lattice and in the continuum. The lattice dimer result is detailed in  \ref{sec:dimers_exact} >>> any more comments to add about this???  For the free-boson field theory, these partition function are known explicitly \cite{BigYellowBook};  we will discuss their application to the two-cylinder Renyi entropy (and some important subtleties) in the next section \ref{sec:dimers_entanglement}. In particular, we will show how the even-odd effect is a signature of the ``locked phase'' for $n>1$. 
%In this phase the even-odd effect is proportional to $n/(n-1)$, and therefore slightly diminishes with $n$. 
...
The main subtlety in this computation is how to account in the field theory for the distinct results occurring when the cylinders are of even and of odd length, as apparent the numerical results in fig.\ \ref{fig:ee_d_torus} for the dimer RVB state and those in \cite{Ju2012} for the $SU(2)$ RVB state. The reason this is possible in the field theory is a ``fixed'' boundary condition is in fact a family of boundary conditions, corresponding to the specific value $h_0$ that the field takes at a boundary. This value can be always be shifted overall, since the bulk action used to compute the partition functions is independent of it. However, on a cylinder, there are two separate boundaries, and what cannot be shifted away is the {\em difference} of the values on the two ends.    
...
%More precisely, the coarse-grained height difference between top and bottom is given by
%\begin{equation}
% \Delta h =h_{\rm top}-h_{\rm bottom}=2\pi (w+a)\quad,\quad w \in \mathbb{Z},
%\end{equation}
%where $a=0$ for even $L_y$ and $a=1/2$ for odd $L_y$. 
%Such a small shift has a tremendous impact in the continuum limit.
...
more words...
 ...
 The partition function may be computed using the standard decoupling of the action into an oscillator part, and a classical part, which is subject to compactification. See for example \cite{EggertAffleck,FSW,BigYellowBook} for more details. 
...
\end{equation}
 ...
 The boundary conditions at the top and bottom correspond to flat configurations for the microscopic heights, which are expected to renormalize to a Dirichlet boundary condition ($D$) for the height field $h$. Such a partition function is well known for a free boson CFT\cite{FSW,EggertAffleck}:
\begin{equation}\label{eq:cyl_dd}
 \mathcal{Z}_{\rm cyl}^{DD}(\tau)=\frac{\theta_3(2\tau)}{\eta(2\tau)},
\end{equation}
and can even be derived from the exact lattice partition function, see \ref{sec:dimers_exact}. The Jacobi theta and Dedekind eta functions are defined in \ref{sec:CFT_Jacobi}. It turns out this reasoning is only valid when the height of the cylinder is even. 
...
Droping the non-universal bulk and boundary free energies in Eqs.~(\ref{eq:aftert}) and (\ref{eq:pmax}) allows to recover Eq.~(\ref{eq:shape_general}). 




where $\tau=i\L_x/L_y$, the $i$ included to conform with standard CFT conventions.
CFT partition functions on cylinders and tori depend on only the aspect ratio of the two lengths; the bulk term depending on the area is independent of the boundary conditions, and so can be factored out of the partition functions and cancels in our formula.
\begin{equation}\label{eq:shape_general}
...
\begin{figure}[ht]
 ...
 \end{center}
\caption{Spin-spin correlation function on the torus. We look at two spin in the (?) direction, separated by a distance $
ell_x$. They decay exponentially, with an exponent that diminishes with $N$. We have $\xi(N=2)=1.30(5)$ (compatible with \cite{RVB1}) and the new $\xi(N=3)=0.68(5)$, $\xi(N=4)=0.56(3)$ and $\xi(N=5)=0.46(2)$.}
...
\end{equation}
at distances large compared to the lattice spacing, but small compared to the torus size $L=L_x=L_y$. Using several different methods, the exponent was found to be approximately $\alpha \simeq 1.2$, and independent on the direction.
%\begin{equation}\label{eq:decay}
% D_{||}(\ell) \sim (\ell/L)^{-\alpha},
%\end{equation}
%in the limit $1\ll \ell \ll L$.
We extend some of these results, on the $SU(N)$ case, mainly focusing on the correlations in the transverse (?) direction ($\vec{r}_1-\vec{r}_2=\ell_x \vec{u}_x$). 
...
Since $SU(N)$ dimer correlations reduce to pure dimer correlations in the limit $N\to\infty$, we have
\begin{equation}
 \lim_{N \to \infty} \alpha_N=\alpha_{\rm D}=2,
\end{equation}
 and it is reasonable to expect that the exponent will interpolate smoothly between the two values $\alpha_2$ and $\alpha_D$.
\begin{figure}[ht]
 \begin{center}
