...
\begin{equation}
 ...
 \label{dimerdimer}
 \end{equation}
\textcolor{blue}{The exponent is $\alpha=2$,\cite{FisherStephenson}. The coefficient $C$ depends on both the direction and the parity of the number of steps along any path connecting the two dimers.} In fermionic language, the dimer creation operator is a fermion bilinear, and so is of dimension 1. 
...
This scenario is also supported by exact diagonalizations on small systems, which show that the biggest eigenvalue of the reduced density matrix is non-degenerate:  $\Delta_{SU(2)}\equiv -\ln (p_1/p_{\rm max}) \approx 2.67$ for a $4\times 4$ torus cut in two halves, and therefore $d_{\rm min}=1$. (The exact value for dimers is $\Delta_{\rm dimers}=2\ln \pi\simeq 2.29$ in the scaling limit \cite{Stephan2012}.). \textcolor{blue}{Such a large entanglement gap has also been observed recently for the $SU(2)$ RVB state, in the closely related infinite cylinder geometry\cite{Poilblanc}.} 
