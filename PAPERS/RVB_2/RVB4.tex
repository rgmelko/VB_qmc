...
In the continuum, these partition functions become those of the free-boson conformal field theory, and so putting (\ref{eq:pmax}) together with (\ref{eq:aftert}) gives (\ref{eq:shape_general}). The boundary conditions at the ends of the cylinder are fixed, i.e.\ at each end the field is set to some value $h_0$. Finding precisely which values is an important subtlety we discuss in the next section \ref{sec:dimers_entanglement}.


For the dimer RVB state, the partition functions on the cylinder can be computed exactly both on the lattice and in the continuum. The lattice dimer result is detailed in  \ref{sec:dimers_exact} >>> any more comments to add about this, I assume you checked numerically that (\ref{eq:after}) holds???  For the free-boson field theory, these partition function are known explicitly \cite{BigYellowBook};  we will discuss their application to the two-cylinder Renyi entropy in the next section \ref{sec:dimers_entanglement}. In particular, we will show how the even-odd effect is a signature of this locked phase  $n>n_c$. 
...
Setting  $\kappa=1/2$ to get dimers, the even case $a=0$ can be written as
\begin{equation}\label{eq:cyl_dd}
 \mathcal{Z}_{\rm cyl}^{DD}(\tau)=\frac{\theta_3(2\tau)}{\eta(2\tau)}\ ,
\end{equation}
where we adopt the conventional variable  $\tau=i L_y/L_x$. 
...
Plugging these results in Eq.~(\ref{eq:shape_general}), and using the torus partition funtion\cite{Ginsparge,BigYellowBook} also given in the appendix, we get for the two-cylinder entanglement entropies
...
\end{figure}
\paragraph{}
The even-odd effect is much less pronouced in for larger $L_y/L_x$, compatible with our prediction that $S_2$ goes exponentially fast in $L_y/L_x$ to a single flat curve. The relatively large finite-size effects should disappear in the thermodynamic limit.
