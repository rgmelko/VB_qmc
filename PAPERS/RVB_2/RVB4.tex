...
\begin{figure}[t]
 \begin{center}
    \scalebox{.35}{\includegraphics[width=\columnwidth]{./figures/updates.pdf}}
 \end{center}
...
As shown by our data, some estimators for this large $N$ show signs of ergodicity loss; this might be cured by a more sophisticated sampling scheme in the future.
...
Note that, when performing the Monte Carl update on the valence-bond state $| D_{\mathcal{R} '} \rangle$, one does not have to consider the overlap of $\langle V_{\mathcal{L}} | V_{\mathcal{R}'} \rangle$ since the overcompleteness condition ensures that the inner product of new valence-bond configurations (with the same spin configuration) are always non-zero.  Hence, it maybe convenient to consider the ``un-swapped'' state $|V_{\mathcal R} \rangle$ for the purposes of sampling the VB configuration, where one can straightforwardly perform local (plaquette-flip) or non-local (loop) updates on the VB state as described in 
...
\begin{eqnarray}\label{eq:cft_w0_gen}
 s_{n,0}^{\rm (even)}&=&\frac{n}{1-n}\ln \left(\frac{\alpha}{2}\frac{\eta(\tau)^2\theta_3(\alpha y(1-y)\tau)}{\eta(2y\tau)\eta(2(1-y)\tau)}\right)\\\label{eq:cft_w0_gen2}
...
In this paper we have studied variations of the archetypical family of resonating-valence-bond (RVB) states on the square lattice, which are the simplest wavefunctions exhibiting gapless spin-liquid-type behavior. %, using analytical and numerical techniques.
We consider $SU(2)$ RVB states, where the wavefunction is an equal-amplitude superposition of nearest-neighbor singlets, amenable to sampling with advanced quantum Monte Carlo techniques; as well as orthogonal dimer-RVB wavefunctions, for which exact calculations of correlation functions can be performed.  

Crucially, it is possible to interpolate between the $SU(2)$ and dimer wavefunctions by generalizing the former case to $SU(N)$ symmetry, for which the two are equal when $N \rightarrow \infty$.  Monte Carlo simulations are able to reach $N=$ with only moderate computational effort. This interpolation can be demonstrated by calculating dimer-dimer correlation functions.  Previous studies demonstrated that the $SU(2)$ RVB state exhibited algebraic decay with an exponent $\alpha_2 \approx 1.2$, while the dimer RVB has $\alpha_{\infty} = 2$.  We showed here that this exponent evolves smoothly,  $\alpha_3=1.43(7)$, $\alpha_4=1.54(8)$, $\alpha_5=1.67(9)$, 
with no evidence for a transition as $N$ is varied.  This is strong evidence of universality, allowing us to link some physical aspects of the $SU(2)$ case with exact results on the dimer RVB.

Some of the most striking behavior of the RVB wavefunctions are manifest in their quantum entanglement properties.  
By measuring the Monte Carlo expectation value of a ``swap'' operator on a replicated lattice, we calculate the second Renyi entropy, which quantifies the entanglement between two subregions of the system.  
In this way, we confirm the previous observation that $S_2$ for the $SU(2)$ RVB exhibits a remarkable two-branch structure when the length of the entangled region varies between and even and odd number of lattice sites.  We are also able to calculate general order Renyi entropies, $S_n$, numerically in the dimer RVB wavefunction, and demonstrate that the two-branch structure exists for $n \ge 2$ (but not for $n<2$).  It would be interesting to examine this on future Monte Carlo calculations of the $SU(2)$ RVB state, where replica-trick calculations of Renyi entropies for $n >2$ are also possible.

Remarkably, the two-branch shape of the Renyi entropies has an important consequence: namely, strong-subadditivity is violated for the ``upper'' branch, which is a concave-up function of the linear size of the entangled region.  We demonstrate that this is far from a numerical artefact by calculating the shape of these branches ....
%XXX
...
\paragraph{Acknowledgments}
 We would like to thank Matthew Hastings for early collaboration on this project. We also thank Ann Kallin, Eduardo Fradkin, Gr\'egoire Misguich, Vincent Pasquier and \ldots for stimulating discussions. This work has been supported by the Natural Sciences and Engineering
Research Council of Canada (NSERC).  Simulations were performed on the computing facilities of SHARCNET.
