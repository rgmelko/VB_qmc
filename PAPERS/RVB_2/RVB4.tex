...
The behavior of quantities such as the R\'enyi entropy thus can be used to characterize spin-liquid states, without requiring knowledge of any specific Hamiltonian. The quantum spin liquids best characterized theoretically are {\it gapped}, where a gauge symmetry typically emerges.
...
We study a particular family of states, resonating-valence-bond (RVB) states on the square lattice, that are gapless and exhibit spin-liquid-type behavior \cite{RokhsarKivelson,RVB1,RVB2}. Our main (but not only) tool is to analyze a subleading term in the R\'enyi entropy particularly useful for characterizing gapless systems \cite{Ju2012}. The R\'enyi entropies $S_n$ are defined for $n\neq 1$
\begin{equation}
...
\textcolor{red}{
This effect, whereby a R\'enyi entropies may exhibit qualitatively different behavior for some higher $n$, is reminiscent of the manifestation of finite-temperature criticality in R\'enyi entropies for $n>1$, but not $n=1$} \cite{RenyiXing}. 
...
which behaves asymptotically as $\sim (1.79162\ldots)^{\cal N}$ \cite{Kasteleyn}.
 A classic result of classical statistical mechanics is that any correlator in the classical dimer model on any planar lattice can be expressed as a Pfaffian \cite{Kasteleyn,Fisher}. This is equivalent to saying that model can be rewritten in terms of free fermions \cite{Samuel}. For the dimer-dimer correlation function, the asymptotic behavior can easily be obtained using this expression. For the square lattice long ago this was shown to be algebraically decaying \cite{FisherStephenson}:
\begin{equation}
  C_{dd}(\mathbf{r}_1,\mathbf{r}_2) \sim \left|\mathbf{r}_1-\mathbf{r}_2\right |^{-\alpha} \ .
 \label{dimerdimer}
 \end{equation}
When $\mathbf{r}_1$ and $\mathbf{r}_2$ are an odd distance apart (i.e. any path along the links of the lattice has an odd number of steps), the exponent is $\alpha=2$\,\cite{FisherStephenson}. In fermionic language, the dimer creation operator is a fermion bilinear, and so is of dimension 1. 
Note that algebraic decay occurs only for bipartite lattices; for non-bipartite lattices, they are exponentially decaying, a fact crucial in showing that the triangular-lattice quantum dimer model is in a gapped RVB phase, i.e.\ has topological order \cite{Moessner}.
...
A well established and very useful fact about the classical dimer model is that long-distance properties of the dimers are described by a free bosonic field theory, or Coulomb gas. This can be seen either by taking the continuum limit of the fermonic description and then bosonizing the fermions, or by rewriting the dimers in terms of a discrete ``height'' degree of freedom \cite{Heights_1,Heights_2,Henley}. We follow the latter approach, because a careful treatment of the boundary conditions is necessary to obtain our results for the two-cylinder R\'enyi entropy. Before going any further, we also note that the same Coulomb-Gas techniques can be used on the six-vertex model. We will however explain this approach only on the dimer model. 
...
In this framework the quantum dimer and quantum six-vertex models can also be written as in terms of a free scalar field in the 2+1 dimensional ``quantum Lifshitz'' model \cite{Henley,QuantumLifshitz}. This is a field theory version of an RK Hamiltonian, and so where the equal-time correlators of a 2d {\em quantum} model are those of a 2d conformal field theory. This model is critical but not Lorentz-invariant, with dynamical critical exponent $z=2$.
...
\begin{equation}
 ...
 \label{Spsm}
\end{equation}
We name this quantity the R\'enyi-Shannon entropy in the following. Note that here $n$ need not be an integer. The probabilities can be conveniently rewritten in terms of dimer partition functions as 
\begin{equation}
...
\begin{enumerate}
 ...
 \item A certain type of local constraint \cite{Shannonee}, which for ${\mathbb Z}_2$ variables on the bonds of the square lattice (e.g.\ dimer occupancies) amounts to:  if three of the variables around each site are specified, then the state of the fourth is uniquely determined.  
\end{enumerate}
Quantum dimers obviously satisfy both these requirements. The same goes for the quantum six-vertex state, (ii) being guaranteed by the ice rule. The $SU(N)$ RVB wave functions satisfies (ii) but not (i) (see (\ref{eq:nonortho})). As an example of Coulomb-gas wave functions that satisfy (i) but not (ii), one can consider the quantum dimer wave function built out of the interacting dimer studied in \cite{Alet_dimers1,Alet_dimers2}. 

Returning to the dimer RVB state, these expressions make it possible to use the computer to find very accurate values for the R\'enyi entropy. Using free fermion techniques described in \ref{sec:lgv}, each probability can be expressed as a product of determinants of two matrices of size $\sim L/2$, and computed numerically in a time of order $\approx L^3$. Using translational symmetry along the $x$ axis as well as the conservation of winding numbers, we are able to compute any R\'enyi entropy up to machine precision in a time of order $L^{3/2}\times 4^L$.
...
Standard Coulomb gas/field theorycomputations (see e.g.\ \cite{Nienhuis}) then give the boundary scaling dimension of (\ref{Vdef}) to be $d^2/(2n\kappa)$. 
...
We checked system sizes up to $L_x=704$, and that found that while indeed there are strong finite-size effects for small system sizes, the data converges in the end to the CFT result. 
...
To study $SU(N)$-RVB wavefunctions, we use an unbiased Monte Carlo technique that calculates expectation values in the equal-amplitude superposition of states using a Metropolis importance sampling scheme.  The RVB wavefunction is represented in a combined VB-spin basis, $|V_{\mathcal C} \rangle = |D \rangle |Z \rangle$, where $| D \rangle$ is a list of site pairs representing the end of the valence bonds, and $|Z \rangle$ is the spin state on each site on the lattice.

In order to calculate expectation values (as described below), two RVB wavefunctions are sampled simultaneously, $\langle V_{\mathcal C} |$ and $| V_{\mathcal C'} \rangle$.  The two VB states $\langle D |$ and $| D' \rangle$ can initially be chosen at random, then spin state compatible with a non-zero overlap $\langle V_{\mathcal C} | V_{\mathcal C'} \rangle$ is constructed.  Since the spin basis is orthogonal, $|Z \rangle = | Z' \rangle$.  Then, Monte Carlo sampling scheme proceeds in two distinct steps.   

After initializing the wavefunction in one configuration $|V \rangle | Z \rangle$, the first step is to sample a new spin state $|Z  \rangle = |Z' \rangle$.  This is done at random, subject to the condition that the overlap $\langle V_{\mathcal C} | V_{\mathcal C'} \rangle$ remains non-zero.  The second step is to sample the VB states $|D \rangle$ and $|D' \rangle$.  This must be done subject to the constraint imposed by the spin states.
Like employed previously for the $SU(2)$ case \cite{Ju2012}, one has the choice of ``local'' or ``non-local'' updating schemes to sample all possible VB states \cite{sandvik2010loop}.
\begin{figure}[t]
 \begin{center}
    \scalebox{.3}{\includegraphics[width=\columnwidth]{./figures/updates.pdf}}
 \end{center}
 \caption{Possible local updates in a simulation....}
 \label{fig:updates}
 \end{figure}
%
...
%
However, if the spin-basis is not compatible with this proposed change, as in Fig.~\ref{fig:updates}(b-c) where the right pointing arrow is a different flavor spin for an $SU(N>2)$ system (e.g. $m_i=0$ for $SU(3)$), the move is rejected and a new ``plaquette flip" is proposed by the simulation.
%
While simple to perform, this move is not ergodic in that it cannot change the winding number of the system.
%
Therefore, this move is most useful if one wants to fix the winding number to do measurements (e.g. for the correlation measurements below, we fix $W=(0,0)$).
%
...
The important point to note is the the Monte Carlo update for the RVB wavefunction proceeds in two steps: sampling the spin state $|Z \rangle$, and sampling the VB states $|D \rangle$.  Each basis configuration is constrained by the other such that the valence-bond remains a quantum $SU(N)$ singlet.  In contrast, for the classical dimer RVB wavefunction, no spin state exists, and dimer wavefunctions are orthogonal.
...
Given Eq.[9], the singlets on a pair of sites $[i,j]$ can be written as
%
\begin{eqnarray}
...
\end{eqnarray}
%
While sampling the spins, there are $N$ choices to choose for a site on sublattice A.
Once this spin is chosen, all other spins in the same loop are determined by Eq.~\ref{eq:su2su3}.
For instance, consider the $SU(2)$ wavefunction in Fig. blah.
There are \textit{two} types of loops: one with $\uparrow$ (loop 1) and one with $\downarrow$ (loop 2) on sublattice A.
...
\begin{figure}[t]
 \begin{center}
    \scalebox{.7}{\includegraphics[width=\columnwidth]{./figures/su2su3graphs.pdf}}
 \end{center}
 \caption{Transition graphs....}
 \label{fig:su2su3graphs}
 \end{figure}
%
Then, generalizing this a little further, consider Fig[SU(3)].
One can easily see that there are three loop types: $\uparrow$, $\downarrow$, and 0 on sublattice A.
So, for an $SU(N)$ system, there are $N$ different loop types, and we quickly run into ergodicity issues as updates are rarely accepted.
...
As show by our data, in this regime, some estimators show signs of ergodicity loss; this might be cured by a more sophisticated sampling scheme in the future.
...
We extend some of these results, on the $SU(N)$ case, mainly focusing on the correlations for two parallel dimers in the direction ($\mathbf{r}_1-\mathbf{r}_2=\ell_x \mathbf{e}_x$). 
...
A naive sampling of the $Swap_A$ operator is straightforward.  Once simply identifies the spatial region $A$ and its complement $B$, then applies the operator to a sampled RVB state, $Swap_A |V_{\mathcal C} \rangle =  |V_{\mathcal C^{\star}} \rangle$.  The second Renyi entropy can be calculated from \cite{swap}
\begin{equation}
	\rho_A^2
		= \frac{\langle V_{\mathcal{L}} | V_{\mathcal{R}^{\star}} \rangle}
			{\langle V_{\mathcal{L}} | V_{\mathcal{R}} \rangle} 
			 = 2^{n_{\rm swap} - n_l}\ ,
\end{equation}
where $n_l$ is the number of loops in the transition graph of the two un-swapped RVB states (denominator), and $n_{\rm swap}$ is the number of loops in the transition graph using one swapped state.  
...
since the $Swap_A$ operator servers to swap {\it all} basis states in the spatial region $A$, spin states $|Z_{\mathcal C} \rangle$ can effectively be ignored when performing this measurement; they are only used as a condition to {\it sample} the original un-swapped VB configurations.

However, as one discovers quickly as the lattice size (and particularly the size of region $A$) grows, sampling statistics become exponentially poor when using the naive $Swap_A$ operator to calculate $S_2$.  One must therefore employ some variant of the {\it ratio} trick described in  Ref.~\cite{swap}.  As discussed in detail in that reference, this involves sampling 
\begin{equation}
...
two states, $|V_{\mathcal L} \rangle = |D_{\mathcal L} Z_{\mathcal L} \rangle$ and $|V_{\mathcal R'} \rangle = |D_{\mathcal R'} Z_{\mathcal R'} \rangle$.  However, the simulation is performed with ${\langle V_{\mathcal{L}} | Swap_X | V_{\mathcal{R}} \rangle}$ in the weight, 
meaning that $ |D_{\mathcal R'} Z_{\mathcal R'} \rangle = Swap_X |D_{\mathcal R} Z_{\mathcal R} \rangle  $.  Therefore, it is important to sample spin states such that the overlap $\langle Z_{\mathcal L} | Z_{\mathcal R '} \rangle$ is nonzero.  

Note that, when performing the second type of update on the valence-bond state $| D_{\mathcal{R} '} \rangle$, one does not have to consider the overlap of $\langle V_{\mathcal{L}} | V_{\mathcal{R}'} \rangle$ since the overcompleteness condition ensures that new valence-bond configurations (with the same spin configuration) are always non-zero.  Hence, it maybe convenient to consider the ``un-swapped'' state $|V_{\mathcal R} \rangle$, where one can perform local (plaquette-flip) or non-local (loop) updates on the VB state as described in 
Section \ref{sec:numerics}.

We employ ratio-trick evaluations of the $Swap_A$ operator to calculate the scaling of the second Renyi entropy in the $SU(N)$ RVB wavefucntion.  In the next section, we compare our results from these Monte Carlo simulations to results obtained from conformal field theory.
...
Let us first begin with the $SU(2)$ case, and present the computation of $S_2$, and also $S_2$ restricted to the $W=(0,0)$ winding sector. The quantum Monte Carlo data is shown in Fig.~\ref{fig:SU2_shape}.\begin{figure}[ht]
 ...
 \end{center}
\caption{Numerical extraction of the universal shape of $S_2$ as a function of $y=\ell_y/L_y$ for the $SU(2)$ RVB wave function and $L=L_x=L_y=12,18,24$. Red and blue curves are the CFT prediction, slightly anticipating on Eqs.~(\ref{eq:cft_prediction_gen}) and (\ref{eq:cft_prediction_gen_odd}). \textcolor{blue}{Another plot right next to this one for the $SU(2)$ $W=(0,0)$ sector.}}
\label{fig:SU2_shape}
\end{figure}
We observe a striking similarity with the dimer data. In particular, a strong even-odd effect which becomes even bigger in the $W=(0,0)$ sector.
This suggests $S_2$ for the $SU(2)$ RVB state lies in a locked phase, just as its dimer counterpart does. 
This scenario is also supported by exact diagonalizations on small systems, which show that the biggest eigenvalue of the reduced density matrix is non-degenerate:  $\Delta_{SU(2)}\equiv -\ln (p_1/p_{\rm max}) \approx 2.67$ for a $4\times 4$ torus cut in two halves. (The exact value for dimers is $\Delta_{\rm dimers}=2\ln \pi\simeq 2.29$ in the scaling limit\cite{Stephan2012}) This suggests a large entanglement gap, along with $d_{\rm min}=1$.

Throughout this paper we have emphasized the similarities between the $SU(N)$ and dimer RVB states. These make it plausible that the two-cylinder R\'enyi entropy entanglement data for the $SU(2)$ RVB state in the scaling limits can be obtained from the free-boson using (\ref{eq:shape_general}). If this is true, this is a much stronger statement than the results for the critical exponent $\alpha$ as a function of $N$ in the previous section. The reason is that even though correlators in the classical dimer model can be obtained by taking the $N\to\infty$ limit of the loop model defined by the $SU(N)$ RVB inner product, the Hilbert space of the two is completely different; the latter being much larger. Thus a dimer crossing the boundary would carry much more entropy. Moreover, the reduced density matrix for $SU(N)$ has blocks with non-zero total spin, while these are by construction forbidden in the quantum dimer model. Thus it is very unlikely that non-universal quantities will be the same in the two cases. 
...
\begin{eqnarray}\label{eq:cft_prediction_gen}
 s_n^{\rm (even)}(y,\tau)&=&\frac{n}{1-n}\ln \left(\frac{\alpha}{2}\times\frac{\eta(\tau)^2}{\theta_3(\alpha\tau)\theta_3(\tau/\alpha)}\times\frac{\theta_3(\alpha y \tau)\theta_3(\alpha(1-y)\tau)}{\eta(2y\tau)\eta(2(1-y)\tau)}\right)\\\label{eq:cft_prediction_gen_odd}
...
\begin{eqnarray}\label{eq:cft_w0_gen}
 s_{\infty,0}^{\rm (even)}&=&-\ln \left(\frac{\alpha}{2}\frac{\eta(\tau)^2\theta_3(\alpha y(1-y)\tau)}{\eta(2y\tau)\eta(2(1-y)\tau)}\right)\\\label{eq:cft_w0_gen2}
 s_{\infty,0}^{\rm (odd )}&=&-\ln \left(\frac{\alpha}{2}\frac{\eta(\tau)^2\theta_4(\alpha y(1-y)\tau)}{\eta(2y\tau)\eta(2(1-y)\tau)}\right)
\end{eqnarray}
...
\label{fig:SUN_CFT}
\end{figure}
As is apparent from fig.~\ref{fig:SUN_CFT}(left), the curve for the even branch is essentially independent of the $\alpha$ over a large range, and ressembles very much the $1d$ result $S_2\propto\ln \sin (\pi y)$. The shape of the odd curve only weakly depends on $\alpha$, but the curve does appreciably shift in this range. We also observe --fig.~\ref{fig:SUN_CFT}(right)-- that contrary to the dimer case ($\alpha=2$), the two odd curves corresponding to $\alpha=1.2$ are not nearly identical in the $W=(0,0)$ and $W=all$ sectors. This feature is also in good qualitative agreement with the data shown in fig.~\ref{fig:SU2_shape}.
...
Thus while we cannot make a definitive conclusion, our results provide qualitative evidence for Eq.~(\ref{eq:shape_general}) applying to the $SU(2)$ RVB state. They also support the convention of \cite{Ju2012} that the subleading piece of the two-cylinder R\'enyi entropy is universal.
...
\end{equation}
Using our biggest system-size even-odd data in (\ref{eq:even_odd}) yields $\alpha \approx 2.27$ for dimers and $\alpha \approx 1.57$ for $SU(2)$ RVB. These numbers can be improved by extrapolation, but the strong finite-size effects described above prevent an accurate estimate of these numbers. However, 
\section{Conclusion}
...
\item Recall some cool strong subadditivity interpretation.
...
\includegraphics{./figures/free_fermions.pdf}
  \end{center}
  \caption{Illustration of the mapping onto free fermions for open boundary conditions. \emph{Left:} reference configuration with staggered dimers. \emph{Right:} transition graph generated by superimposing the reference configuration on a given dimer configuration (in thick blue). This generates a collection of $4$ non-intersecting lattice paths going upwards, which may be expressed in terms of fermions (black zigag lines). Here $|\sigma\rangle=c_2^\dag c_4^\dag c_6^\dag c_8^\dag|0\rangle$.}
...
with
\begin{equation}
 Z_\nu^{(\pm)}=\prod_{k\in \Omega_{\nu}}\left[e^{\pi x /2}\pm e^{-\pi x/2}\lambda_k^{L_y}\right]
\end{equation}
and
\begin{eqnarray}
...
\end{eqnarray}
Of course, $Z_{\rm torus}(u_x=0,u_y=0)$ reduces to the well known torus partition function obtained by Kasteleyn and Fisher. 
...
\begin{equation}
 \mathcal{Z}_{\rm cyl}= e^{\frac{\pi L_x}{24 L_y}}\prod_{p=1}^{\infty}\left[1+2 \cosh (\pi u_y)e^{-\frac{\pi L_x}{L_y}(p-1/2)}+e^{-2\frac{\pi L_x}{L_y}(p-1/2)}\right]
\end{equation}
which gives, using the infinite product representation of the Jacobi Theta function,
\begin{equation}
 \mathcal{Z}_{\rm cyl}=\frac{\theta_3(i \pi u_y/2|\tilde{\tau}/2)}{\eta(\tilde{\tau}/2)}
\end{equation}
...
\begin{equation}
 \mathcal{Z}_{\rm cyl}= 2 e^{-\frac{\pi L_x}{12 L_y}}\cosh(\pi u_y/2)\prod_{p=1}^{\infty}\left[1+2 \cosh (\pi u_y)e^{-\frac{\pi L_x}{L_y}p}+e^{-2\frac{\pi L_x}{L_y}p}\right]
\end{equation}
In the end we obtain:
\begin{equation}
 \mathcal{Z}_{\rm cyl}=\frac{\theta_2(i \pi u_y/2|\tilde{\tau}/2)}{\eta(\tilde{\tau}/2)}
\end{equation}
...
\begin{eqnarray}\label{eq:nullwinding1}\fl
...
\fl&=&\frac{\theta_2\Big(0\left|\frac{\tilde{\tau}}{2y(1-y)}\right)}{\eta(\frac{\tilde{\tau}}{2y})\eta(\frac{\tilde{\tau}}{2(1-y)})}
\end{eqnarray}
