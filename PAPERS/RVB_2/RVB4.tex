...
The first step in the mapping of classical dimers on to the free-boson Coulomb gas is to associate an integer with each face of the lattice, as illustrated in fig.~\ref{fig:height_shift}.
\begin{figure}[ht]
 \begin{center}
...
Standard Coulomb-gas/field-theory computations (see e.g.\ \cite{Nienhuis}) then give the boundary scaling dimension of (\ref{Vdef}) to be $d^2/(2n\kappa)$. 
...
We extend some of these results to the $SU(N)$ case, mainly focusing on the correlation between two parallel dimers in the direction ($\mathbf{r}_1-\mathbf{r}_2=\ell_x \mathbf{e}_x$). 
