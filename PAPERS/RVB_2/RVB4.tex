...
Plugging these results into Eq.~(\ref{eq:shape_general}), and using the torus partition funtion\cite{Ginsparg,BigYellowBook} also given in the appendix, we get for the two-cylinder entanglement entropies
...
\end{figure}
Obviously, these $n>n_c$ results are quite different in the even and odd sectors; the difference proportional to $n/(n-1)$, and therefore slightly diminishes with increasing $n$, but does not go away. 
...
>>> Is this right? Shouldn't the result be $\propto \ln(L_x)$ for fixed $L_y$???
...
\end{eqnarray}
Even though the total winding number is zero, the result still depends on the compactification radius of the height field  because of winding fluctuations at the boundaries between $A$ and $B$. 
