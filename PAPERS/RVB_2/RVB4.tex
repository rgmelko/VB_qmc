...
updating schemes to sample all possible RVB states \cite{sandvik2010loop}.
In employing local updates, the simulation chooses two parallel valence bonds on a square plaquette and moves each to the empty bond, subject to the condition that the spin-basis is compatible with this new configuration.  This move, while simple to perform, is not ergodic in the winding number sector.
...
\end{equation}
\textcolor{red}{HYEJIN, check this definition and cross-reference with Figure 7.}\textcolor{blue}{Checked!}
...
%
and $0$ otherwise.
\textcolor{red}{HYEJIN, check and cross reference with Figure 6}\textcolor{blue}{done}.
...
\includegraphics{./figures/free_fermions.pdf}
  \end{center}
  \caption{Illustration of the mapping onto free fermions for open boundary conditions. \emph{Left:} reference configuration with staggered dimers. \emph{Right:} transition graph generated by superimposing the reference configuration on a given dimer configuration (in thick blue). This generates a collection of $4$ non-intersecting lattice paths going upwards, which may be expressed in terms of fermions (black zigag lines). Here $|\sigma\rangle=c_1^\dag c_3^\dag c_5^\dag c_7^\dag|0\rangle$.}
...
Define a fermion as a vertical link occupied by a reference (red) dimer only, or a real (blue) dimer only. These are the black zigzag lines in fig.~\ref{fig:freefermions}. $|\sigma\rangle$ and $|\mu\rangle$ can be rewritten using fermion creation operators as
\begin{eqnarray}
