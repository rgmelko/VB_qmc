...
\begin{equation}
 ...
 \label{dimerdimer}
 \end{equation}
%When $\mathbf{r}_1$ and $\mathbf{r}_2$ are an odd distance apart (i.e. any path along the links of the lattice has an odd number of steps), the exponent is
%$\alpha=2$\,\cite{FisherStephenson}.
The exponent is $\alpha=2$,\cite{FisherStephenson}. \textcolor{blue}{[JM: Removed the even-odd subtlety here. Otherwise we would have to discuss the direction dependent dipolar term $(-1)^{\rm number of steps}\cos (2\theta)/|\mathbf{r}_1-\mathbf{r}_2|^2$, which we do later.]}
In fermionic language, the dimer creation operator is a fermion bilinear, and so is of dimension 1. 
...
This expectation was convincingly confirmed numerically \cite{RVB1,RVB2}. The dimer-dimer correlator for dimers was found to be of the form (\ref{dimerdimer}) with
...
A well established and very useful fact about the classical dimer model is that long-distance properties of the dimers are described by a free bosonic field theory, or Coulomb gas. This can be seen either by taking the continuum limit of the fermonic description and then bosonizing the fermions, or by rewriting the dimers in terms of a discrete ``height'' degree of freedom \cite{Heights_1,Heights_2,Henley}. We follow the latter approach, because a careful treatment of the boundary conditions is necessary to obtain our results for the two-cylinder R\'enyi entropy. 
...
In this section, we discuss the shape-dependent subleading contribution\footnote{\textcolor{blue}{Of course, the non universal leading ``area law'' contribution is shape-independent. Possible additionnal universal logarithmic terms, which we don't encounter in this paper, would also be shape-independent.}} to the entanglement entropy 
...
\textcolor{blue}{Note that the non-universal bulk parts of the partition functions cancel in this expression. The non-universal boundary parts don't, and contribute to the area law in (\ref{eq:aftert}). Thus putting (\ref{eq:pmax}) together with (\ref{eq:aftert}) and filtering out the area law term gives (\ref{eq:shape_general}).} 
...
\begin{figure}[t]
 \begin{center}
    \scalebox{.65}{\includegraphics[width=\columnwidth]{./figures/su2su3graphs.pdf}}
 \end{center}
...
We employ ratio-trick evaluations of the $Swap_A$ operator to calculate the scaling of the second Renyi entropy in the $SU(N)$ RVB wavefucntion.  In the next subsection, we compare our results from these Monte Carlo simulations to those obtained from conformal field theory.
...
Crucially, it is possible to interpolate between the $SU(2)$ and dimer wavefunctions by generalizing the former case to $SU(N)$ symmetry, for which the two are equal when $N \rightarrow \infty$.  Monte Carlo simulations are able to reach $N=5$ with only moderate computational effort. This interpolation can be demonstrated by calculating dimer-dimer correlation functions.  Previous studies demonstrated that the $SU(2)$ RVB state exhibited algebraic decay with an exponent $\alpha_2 \approx 1.2$, while the dimer RVB has $\alpha_{\infty} = 2$.  We showed in this work that this exponent evolves smoothly,  $\alpha_3=1.43(7)$, $\alpha_4=1.54(8)$, $\alpha_5=1.67(9)$, 
with no evidence for a transition as $N$ is varied.  This is strong evidence of universality, allowing us to link various physical aspects of the $SU(2)$ case with exact results on the dimer RVB.

Some of the most striking behavior of the RVB wavefunctions are manifest in their quantum entanglement properties.  
By measuring the Monte Carlo expectation value of a ``swap'' operator on a replicated lattice, we calculate the second Renyi entropy, $S_2$, which quantifies the entanglement between two subregions of the system.  
In this way, we confirm the previous observation that $S_2$ for the $SU(2)$ RVB exhibits a remarkable two-branch structure when the length of the entangled region varies between and even an odd number of lattice sites.  We are also able to calculate general-order Renyi entropies, $S_n$, numerically in the dimer RVB wavefunction, and demonstrate that the two-branch structure exists for $n \ge 2$ (but not for $n<2$).  It would be interesting to examine this with future Monte Carlo calculations of the $SU(2)$ RVB state, where replica-trick calculations of Renyi entropies for $n >2$ are also possible.

Remarkably, the two-branch shape of the Renyi entropies has an important consequence: namely, strong-subadditivity is violated for the ``upper'' branch, which is a concave-up function of the linear size of the entangled region.  We demonstrate that this is far from a numerical artefact by calculating the shape of these branches in an effective conformal field theory.  This effective theory involves partition functions that can be evaluated exactly in the continuum, thus allowing access to the Renyi entropy, which exhibits the same remarkable two-branch structure observed in lattice calculations.  This emphasizes the universality of the result and confirms that the two-branch Renyi entropies are not the result of finite-size lattice effects.  From this analysis, we have elucidated two important points regarding the shape of the Renyi entropy as a function of linear size of the entangled region.  

First, the two-branch structure does not occur for every Renyi index $n$.  Rather, it emerges for some critical $n_c$, which must be greater than unity since the von Neumann entropy $S_1$ is strictly constrained by strong-subadditivity (while the $n>1$ Renyi entropies are not).  We believe that in the square lattice RVB, the $1<n_c<2$; thus, Monte Carlo simulations (which only access $n>1$) will always see a two-branch structure as observed here.  
%Resolving this issue is certainly an interesting question for further studies. 
It would be useful to change $d_{\rm min}$ by studying the RVB wave function on other bipartite lattices. For example on the honecomb lattice $d_{\rm min}=3$ (and therefore $n_c=3^2=9$) for dimers, and $S_2$ for RVB would likely lie in the replicated phase, not subject to the even-odd effect. 
This prediction should be tested by future Monte Carlo studies.

Second, we have elucidated an exact expression for the shape of the Renyi entropy curves in a geometry where a linear size of the entangled regions grows as $L$.  As observed in previous studies of gapless systems \cite{Ju2012}, this was well approximated by the celebrated one-dimensional result $ln(L)$; however, deviations were observed in the numerical data.  Using our effective field theory we have obtained an exact expression, which is demonstrated to be consistent with our numerical data for the $SU(2)$ and dimer RVB wavefunctions, commensurate with the notion that it is universal.  This conjecture should be tested on future gapless wavefunctions.  \textcolor{red}{note: I have data for the 2D TFIM critical point... will this work there?}
%It could also be extended to other RK points described by other CFTs, generalizing the results of\cite{Stephan2010,Zaletel}

In summary, we have used analytical and numerical techniques to study entanglement and correlations in $SU(N)$ and dimer RVB wavefuctions --  the simplest wavefunctions exhibiting features of gapless spin-liquid behavior.  Since gapless spin liquids are long-range entangled states {\it not} amenable to  classification by simple quantities such as the Topological Entanglement Entropy, we believe that some of the universal scaling properties uncovered here will be of use in future efforts to classify and characterize them in theory, numerics, and experiment.
