...
In this paper we do not address this question directly. Instead we focus on the gapless behavior of nearest-neighbor RVB states on the square lattice. One key tool is to study RVB state of the quantum dimer model. The Hilbert space of the quantum dimer model is spanned by dimer states, i.e.\ each dimer configuration is a basis element of this Hilbert space. The RVB state for dimers is then the equal-amplitude sum over all these states: cite{RokhsarKivelson}.
\begin{equation}
...
\begin{equation}
 ...
 \end{equation}
In the QDM one essentially forgets the underlying spins: the only degrees of freedom in the quantum dimer model are the nearest-neighbor dimers.



The important difference between the $SU(2)$ RVB state (\ref{RVBdef1},\ref{RVBdef2}) is the inner product. 
In the quantum dimer model, different dimer configurations are defined to be orthogonal:
$$\langle D_{\beta} | D_{\alpha} \rangle = \delta_{\beta,\alpha}\ .$$ 
In the RVB state, the inner product is the usual one for spins: each different spin state in the $S^z$ diagonal basis is orthogonal to the others. As is easily shown \cite{Sutherland}, this means that different dimer configurations in the spin model are {\em not} orthogonal. In fact, there is a convenient loop-gas description to represent their inner product. One places the two dimer graphs on top of each other. Because of the requirement that there be exactly one dimer per site, this means that the dimers of the two graphs put together form closed loops. The shortest loops are of length two, when both graphs have a dimer on the same link. Using the spin inner product then gives 
