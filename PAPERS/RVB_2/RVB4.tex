...
In this paper we studied a family of resonating-valence-bond states on the square lattice, the simplest wavefunctions exhibiting gapless spin-liquid-type behavior. We focused in particular on the Renyi entropy $S_n$ from splitting a torus into two cylinders, which provides an elegant probe into the physics of gapless states. We found several interesting characteristics of their quantum entanglement, by deriving exact results for the dimer RVB  state, and by numerical work on the $SU(2)$ and the $SU(N)$ RVB states. 

One of our main results is confirmation that the Renyi entropy for RVB states exhibits a two-branch structure, when the length of the entangled region varies between and even an odd number of lattice sites. We computed this exactly for the dimer RVB functions by mapping the Renyi entropy to a classically-computable Shannon entropy, and demonstrated that the two-branch structure exists for $n \ge 2$ (but not for $n<2$).
By measuring the Monte Carlo expectation value of a ``swap'' operator on a replicated lattice, we find $S_2$ for the $SU(N)$ RVB state for $N=2,3,4$, and find the same two-branch structure. It would be interesting to examine this with future Monte Carlo calculations of the $SU(2)$ RVB state, where replica-trick calculations of Renyi entropies for $n >2$ are also possible.


The two-branch shape of the Renyi entropies has an interesting consequence:  strong-subadditivity is violated for the ``odd'' branch, which is a concave-up function of the linear size of the entangled region.  We demonstrate that this is far from a numerical artefact even away from the dimer case, by calculating the shape of these branches in an effective conformal field theory.  This effective theory involves partition functions that can be evaluated exactly in the continuum, and exhibit the same two-branch structure observed in lattice calculations.  This emphasizes the universality of the result and confirms that the two-branch Renyi entropies are not the result of finite-size lattice effects.  

From this analysis, we have elucidated two important points regarding the shape of the Renyi entropy as a function of linear size of the entangled region.  
First, the two-branch structure does not occur for every Renyi index $n$, but only for values larger than some (non-universal) $n_c$, which must be greater than unity since the von Neumann entropy $S_1$ is strictly constrained by strong-subadditivity (while the $n>1$ Renyi entropies are not).  We believe that in the square lattice RVB, $1<n_c<2$; since our Monte Carlo methods require $n\ge 2$, they will always find a two-branch structure as observed here.  
...
Second, we have elucidated an exact expression for the shape of the Renyi entropy curves in a geometry where a linear size of the entangled regions grows as $L$.  As observed in previous studies of gapless systems \cite{Ju2012}, this was well approximated by the celebrated one-dimensional result $ln(\sin(\pi y))$; however, deviations were observed in the numerical data. We have shown that while this is a similar curve, it is indeed only approximate. Using our effective field theory we have obtained an exact expression, which is demonstrated to be consistent with our numerical data for the $SU(2)$ and dimer RVB wavefunctions, commensurate with the notion that it is universal.  his conjecture should be tested on future gapless wavefunctions.  \textcolor{red}{note: I have data for the 2D TFIM critical point... will this work there?}
\textcolor{blue}{Take a look at Konik's talk, he claims this behavior, URL in a comment here}
%www.ggi.fi.infn.it/index.php?p=schedule.inc&idev=106}
...
We have also explored the crossover between the $SU(2)$ and dimer RVB states by studying the $SU(N)$ states and taking $N\to\infty$. For equal-time correlators, it is possible to interpolate between the $SU(2)$ and dimer states by taking $N \rightarrow \infty$; the issue is whether   We showed evidence that this correspondence still holds for universal parts of the Renyi entropy. Previous studies demonstrated that the $SU(2)$ RVB state exhibited algebraic decay with an exponent $\alpha_2 \approx 1.2$, while the dimer RVB has $\alpha_{\infty} = 2$.  We showed in this work that this exponent evolves smoothly as $N$ is varied,
with no evidence for a transition.  
Monte Carlo simulations are able to reach $N=5$ with only moderate computational effort. 
This is strong evidence of universality, allowing us to link various physical aspects of the $SU(2)$ case with exact results on the dimer RVB.
