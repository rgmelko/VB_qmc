...
<<<<<<< .mine
We extend some of these results to the $SU(N)$ case, mainly focusing on the correlations in the transverse (?) direction ($\vec{r}_1-\vec{r}_2=\ell_x \vec{u}_x$). 
>>> say the dimers are parallel to each other and perpendicular to $r_1-r_2$ ???
=======
...
Our data clearly indicate that the algebraic decay remains for $N=2,3,4,5$, with $\alpha$ increasing with $N$. 
