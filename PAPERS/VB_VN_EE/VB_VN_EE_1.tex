...
The properties of $S^{\rm VN}_A$ are well-studied in the field of quantum information theory.
...
Tensor-network extension of MPS states form a promising new class of numerical algorithms \cite{PEPS1,PEPS2} 
...
the $S^{\rm VN}_A$, $S^{\rm VB}_A$ can be accessed easily in the valence-bond basis
projector QMC method recently proposed by Sandvik \cite{Sandvik}.  As
demonstrated in Refs.~\cite{Alet,Chh}, $S^{\rm VB}_A$ has many properties in
common with $S^{\rm VN}_A$, in particular the relationship $S_A = S_B$, and the
fact that $S^{VB}_A=0$ for regions ``un-entangled'' by valence bonds.
A comparison of the scaling of $S^{\rm VB}_A$ for (critical) 1D spin
...
true also for the $S^{\rm VN}_A$, this fact would have negative consequences for the simulation of the 
...
In this paper, we compare $S^{\rm VB}_A$ calculated by valence-bond QMC to 
$S^{\rm VN}_A$ accessible through density-matrix renormalization group
(DMRG) simulations of the Heisenberg model on $N$-leg ladders.    For $N=1$, the CFT central charge calculated  by scaling
$S^{\rm VN}_A$ shows excellent agreement to $c=1$, whereas $S^{\rm VB}_A$ converges
to $c<1$.
For $N>1$, $S^{\rm VB}_A$ is systematically greater than $S^{\rm VN}_A$,
...
$N=6$, the VN EE convincingly shows a scaling of
...
only a lower bound for $S^{VN}_{A}$ is calculated, so care must be taken
...
For PBC both
$S^{\rm VB}_A$ and $S^{\rm VN}_A$ are seen to fit well to the CFT result, although
$S^{\rm VN}_A > S^{\rm VB}_A$. The regression fit shows very good
convergence with the central charge predicted by CFT, $c=1$, for $S^{\rm VN}_A$, while
$S^{\rm VB}_A$ fit yields a lower value than predicted.
...
For OBC both $S^{\rm VN}_A$ and $S^{\rm VB}_A$ split into two branches, the upper (lower)
...
Notice that contrary to the PBC case, $S^{\rm VN}_A$ is now \textit{smaller}
than $S^{\rm VB}_A$. A regression fit of the lower branch to the form $c/6 \ln
({2x'})$ (inset) shows excellent convergence of $S^{\rm VN}_A$ to the central
...
$S^{\rm VB}_A$ fit deviates significantly from the CFT result for larger system
...
Figure~\ref{ladder} shows $S^{\rm VN}_A$ and $S^{\rm VB}_A$ calculated
for the 3-leg and 4-leg ladder. As happens with the OBC 1D chain, $S^{\rm VB}_A > S^{\rm VN}_A$ .  
...
Plotting $S_A/N$ versus $N$ on a log scale, one sees that we obtain a
...
the plot of the $S^{\rm VN}_A$ data from the DMRG, which convincingly approaches a
constant for large $N$.  Clearly, for $S^{\rm VN}_A$ the area law is
indeed obeyed in the N\'eel groundstate, leading one to conclude that the
multiplicative logarithmic correction occurs in $S^{\rm VB}_A$ only.  We also show data for free fermions, showing the logarithmic correction to the area law
for $S^{\rm VN}_A$ in this case\cite{2dfermion}.
...
divergence to $S^{\rm VN}_A$ since a gapless bosonic mode in 2D
...
$S/N\approx 0.2$, slightly lower than suggested for $S=1/2$ in Fig.~\ref{zigzag}.
...
the valence bond entanglement entropy ($S^{\rm VB}_A$) defined in
Refs.~\cite{Alet,Chh} to the von Neumann entanglement entropy ($S^{\rm VN}_A$) in the
...
In 1D, we find
that $S^{\rm VB}_A$ mimics the behavior of $S^{\rm VN}_A$ closely, although 
%using
%the definition of Eq.~\eqref{VBEE}, 
it is less than $S^{\rm VN}_A$ for periodic
chains, and greater than $S^{\rm VN}_A$ for open chains. In addition, fits to
1D conformal field theory, which are excellent for $S^{\rm VN}_A$ calculated
via DMRG, appear to deviate significantly for $S^{\rm VB}_A$ in the large
chain-size limit, approaching some $c<1$ for both boundary conditions.

On multi-leg ladder systems with open boundaries, $S^{\rm VB}_A$ is
systematically greater than $S^{\rm VN}_A$, a discrepancy which grows as the
...
evaluate the adherence of the entanglement entropies to the area law.  $S^{\rm VN}_A$ obeys the area law in the large-leg limit, whereas $S^{\rm VB}_A$
 reproduces a multiplicative log correction found in
Refs.~\cite{Alet,Chh} for the N\'eel groundstate of the 2D
model.

The fact  that $S^{\rm VB}_A$ can be either greater or less than $S^{\rm VN}_A$ can be understood through simple
...
Then, the state $|(12)(34)(56)(78)\rangle+|(14)(32)(58)(76)\rangle$ has vanishing $S^{\rm VB}_A$ since
no bonds connect $A$ to $B$, but non-vanishing $S^{\rm VN}_A$.  
On the other hand, consider a 4-site chain in
the state $|(12)(34)\rangle+|(14)(32)\rangle$, and let region $A$ contain sites $1$ and $3$.  Then, the $S^{\rm VB}_A$ is maximal,
equal to $2\log(2)$, while $S^{\rm VN}_A$ is smaller.  The second state we have
...
Thus, it is not surprising that states with N\'eel order show $S^{\rm VB}_A > S^{\rm VN}_A$.
...
numerical simulations in 2D and higher, $S^{\rm VB}_A$ does not provide a bound
on $S^{\rm VN}_A$, unlike other entropic measures such as Renyi entropies.
Other properties of $S^{\rm VN}_A$, such as its adherence to an area law, are
...
unphysical multiplicative logarithmic corrections appear in $S^{\rm VB}_A$,
...
be taken into account in proposals to use $S^{\rm VB}_A$ for such important
...
Canada.
% TODO I may need to add support from KITP
I.~G. was supported in part by NSF under Grant No. NSF PHY05-51164. 
