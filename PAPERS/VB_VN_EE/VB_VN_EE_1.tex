...
%tensor network states (and MERA in two dimensions)
...
% on nearest neighbor sites of the ladder.  
a number
...
belonging to region $A$ ($B$) [we can label $S_A$ by its site index,
...
(and subsequent figures) can be considered as exact comparisons between
$S^{\rm VB}_A$ and $S^{ \rm vN}_A$.
...
the region A includes $2N^2$ sites.  All ladders have 100 sites per leg.
\label{zigzag}}} \end{figure}
...
% should we use "full" names ? Does not need an extra line
%M.~P.~A.~Fisher, L.~Hormozi, and E.~S.~Sorensen 
