...
spin-gap~\cite{White1994}, and therefore only sites at distances from the
boundary between A and B smaller than the correlation length $\xi$
...
A qualitative explanation based on a RVB picture is the following. An
even-leg ladder can be fully covered by singlets formed by next-neighbor
spins on the same rung of the ladder. Singlets in neighboring rungs can
then resonate to a configuration where singlets are formed by
next-neighbors sites along the legs, forming ``plaquettes'' (e.~g. sites
11 to 14, in Fig.~\ref{ladder}). Singlets between distant sites freeze the
VB configuration in region they cross, preventing
%TODO or hampering or impeding ? I'll write preventing, but became greedy
%lately... 
this resonance and increasing the energy of the system~\cite{White1994}.
Thus short-range VB are favored in even-leg ladders. On the contrary,
rungs in odd-leg ladders can not be fully covered by singlets, leaving one
site ``un-paired'', and singlets between distant sites are not penalized.
Within this picture, one expects higher EE values when the boundary
between A and B cuts a rung, $x\equiv0 \mod{N}$, than when cuts only legs,
$x\equiv0 \mod{N}$.  Within the same rung, sites at the ends will tend to
form singlets with its only neighbor in the rung and EE will be larger for
$(x\div N) \equiv 1 \mod{2}$, than for $(x\div N) \equiv 0 \mod{2}$,
explaining the period with $N$ for the even-leg ladders. The doubling of
the period for odd-leg ladders is due to the same dimerization effect
discussed in Ref.~\cite{Ian1}.
