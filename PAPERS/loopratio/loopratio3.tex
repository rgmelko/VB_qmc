...
We use the Heisenberg Hamiltonian, which is rewritten in terms of bond operators acting on pairs of sites.
The Hamiltonian can be written as a sum of all possible {\color{red}permutations (with repetition)} of bond operators.
The Monte Carlo algorithm acts to sample terms proportional to their weight in the sum, where the weight depends on the number of off-diagonal operators in the term.
{\color{red}We can use this method to propagate just one state (the single projector method), however with two states (double projector) it is possible to measure expectation values of observables by applying an operator to one of the states, and taking the innerproduct between the resulting state and another propagated state.}


\subsubsection{Loop Algorithm}

\subsection{Measuring the Renyi Entanglement Entropies}
