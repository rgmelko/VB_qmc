// vb_1d.cpp use as reference.  Last updated Wed May 14, 2008
...
main()
{
  ...
  for(int super_n=0; super_n<2; super_n++)
    {
      int bonds[iterations] = {0};
      int acc = 0, rej =0; 
      long int w_old=0, w_new=0;
      double energy = {0};
      if(super_n == 0)bondss = fopen("bonds_A.txt", "w");
      if(super_n == 1)bondss = fopen("bonds_B.txt", "w");
      if(super_n == 2)bondss = fopen("bonds_C.txt", "w");
      if(super_n == 3)bondss = fopen("bonds_D.txt", "w");
      if(super_n == 4)bondss = fopen("bonds_E.txt", "w");
      if(super_n == 5)bondss = fopen("bonds_F.txt", "w");
      if(super_n == 6)bondss = fopen("bonds_G.txt", "w");
      if(super_n == 7)bondss = fopen("bonds_H.txt", "w");
      if(super_n == 8)bondss = fopen("bonds_I.txt", "w");
      if(super_n == 9)bondss = fopen("bonds_J.txt", "w");
      if(super_n == 10)bondss = fopen("bonds_K.txt", "w");
      if(super_n == 11)bondss = fopen("bonds_L.txt", "w");

      int operaters[n[super_n]][2], new_operaters[n[super_n]][2];

  for(int i0=0; i0<n[super_n]; i0++) // generate operators
    {  
      irand();
  ...
  for (int i=0; i<iterations; i++)
    {  
      ...
      for(int i7=0; i7<n[super_n]; i7++)
	{ 
	  new_operaters[i7][0] = operaters[i7][0];
	  new_operaters[i7][1] = operaters[i7][1];
	}

      change_operators(new_operaters, n[super_n]);
