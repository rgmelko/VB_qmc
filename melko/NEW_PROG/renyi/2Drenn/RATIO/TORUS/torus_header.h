		 ...
		 long long rseed)
{
  ...
  //and I'm assuming the dimension is two with PBCs
...
void LATTICE::nnbondlist()
{
  ...
  for(int j=0; j<M; j++){
    for(int i=0; i<L; i++){
      nnbonds(L*M+i+j*L,0) = i + j*L;
      nnbonds(L*M+i+j*L,1) = i + ((j+1)%M)*L;
    }
  }


  for(int b1=2*L*M; b1<4*L*M; b1++){
    int a1 = 2*L*M;
    nnbonds(b1,0) = nnbonds(b1-a1,0)+L*M;
    nnbonds(b1,1) = nnbonds(b1-a1,1)+L*M;
  }
}
