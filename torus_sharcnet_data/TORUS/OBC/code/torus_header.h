		 ...
		 long long rseed)
{
  ...
  number_of_nnbonds = 4*xsites*ysites - 2*xsites - 2*ysites; //changed for OBC
  //and I'm assuming the dimension is two with !!!OBCs!!!
...
void LATTICE::nnbondlist()
{
  int L = xsites;
  int M = ysites;
  int N = number_of_bonds; //number_of_sites/2 since lattice is doubled

  int bondnum = 0;
  //horizontal bonds
  for(int j=0; j<M; j++){
    for(int i=0; i<L-1; i++){ //changed to L-1 for PBC
      nnbonds(bondnum,0) = i + j*L;
      nnbonds(bondnum,1) = j*L + (i+1)%L;
      bondnum++;
    }
  }
  
  //vertical bonds
  for(int j=0; j<M-1; j++){ //Changed to M-1 for PBC
    for(int i=0; i<L; i++){
      nnbonds(bondnum,0) = i + j*L;
      nnbonds(bondnum,1) = i + ((j+1)%M)*L;
      bondnum++;
    }
  }


  for(int b1=bondnum; b1<number_of_nnbonds; b1++){
    int a1 = 2*L*M-L-M;
    nnbonds(bondnum,0) = nnbonds(b1-a1,0)+L*M;
    nnbonds(bondnum,1) = nnbonds(b1-a1,1)+L*M;
    bondnum++;
  }
...
{  
  ...
  vector <int> VLuse = bonds1;
  int a,b,c,d;
 
  for(int A=0; A<xsites; A++){  //Changed...
