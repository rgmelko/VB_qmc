//Febru 19, 2013 --- Modifying code so it does LxL (or Lx2L?) torus
...
void LOOPS::operatorLists()
{
  ...
  //Generic 2D case and 1D ring.
    ...
    if(geometry=="cylinder"){
      ...
      //initialize structures that hold plaquettes & nnbonds
	...
	for(int j=0; j<2*number_of_sites; j++){ nn_mat(i,j) = -99;}
      }
	...
	for(int y=0; y<Ly; y++){
      ...
      //Torus case
      number_of_nnbonds = 2*Lx*Ly;
      numPlaquettes = 2*Lx*Ly;
	...
	for(int y=0; y<Ly; y++){
	  ...
	  plaquettes(pCounter,1) = Ly*x+(y+1)%Ly;
	  //next two sites are shifted right (in the Lx direction)
	  plaquettes(pCounter,2) = Ly*((x+1)%Lx)+y;
	  plaquettes(pCounter,3) = Ly*((x+1)%Lx)+(y+1)%Ly;
	  pCounter++;	  
	}
      }
      
      //xbonds... (horizontal, periodic)
      for(int x=0; x<Lx; x++){
	for(int y=0; y<Ly; y++){
	  nnbonds(bCounter,0) = Ly*x+y;
	  nnbonds(bCounter,1) = Ly*((x+1)%Lx)+y;
	  nn_mat(nnbonds(bCounter,0),nnbonds(bCounter,1))=bCounter;
	  nn_mat(nnbonds(bCounter,1),nnbonds(bCounter,0))=bCounter;
	  bCounter++;

	  plaquettes(pCounter,0)=Ly*x+y;
	  plaquettes(pCounter,1)=Ly*((x+1)%Lx)+y; // i+Ly;
	  plaquettes(pCounter,2)=Ly*x+(y+1)%Ly; //i+1;
	  plaquettes(pCounter,3)=Ly*((x+1)%Lx)+(y+1)%Ly; // i+1+Ly;
	  pCounter++;
	}
      }
    ...
    else{ cout << "geometry error!\n"; exit(1);}   

    // Copy/double the bonds/plaquettes for the replicas
